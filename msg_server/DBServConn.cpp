...
void CDBServConn::_HandleDepartmentResponse(CImPdu *pPdu)
{
    ...
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t body_lenght = pPdu->GetBodyLength();
    uint32_t user_id = msg.user_id();
    uint32_t latest_update_time = msg.latest_update_time();
    uint32_t dept_cnt = msg.dept_list_size();
    log("HandleDepartmentResponse size:%u, user_id=%u, latest_update_time=%u, dept_cnt=%u.", body_lenght, user_id, latest_update_time, dept_cnt);
